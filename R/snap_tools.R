...
advanced_snap <- function(roads, ref, field, tolerance, updatable)
{
  ...
  for (tb_node in ls_tb_nodes)
  {
    ...
    is_valid <- sapply(lines_extended, function(line) {
      # Wrap in sfc and assign CRS
      line <- sf::st_sfc(line, crs = sf::st_crs(roads))
      line <- sf::st_transform(line, sf::st_crs(bridge[["bridge"]]))

      # Compute intersection and handle all geometry types robustly
      inter <- tryCatch(sf::st_intersection(line, bridge[["bridge"]]),
                        error = function(e) return(NULL))

      if (is.null(inter)) return(FALSE)

      # Check for overlapping geometry types
      geom_types <- sf::st_geometry_type(inter)
      if (inherits(inter, "GEOMETRYCOLLECTION")) {
        # Decompose and check if it contains any line-type parts
        inter <- sf::st_collection_extract(inter, "LINESTRING", warn = FALSE)
        if (length(inter) > 0) return(FALSE)
      }

      !any(geom_types %in% c("LINESTRING", "MULTILINESTRING"))
    })

    lines_extended <- lines_extended[is_valid]
    ...
    if(length(dist_bridge)==0){next}
    dist_bridge <- sapply(dist_bridge, function(x) if (length(x) == 0) NA else x)
    if (all(is.na(dist_bridge))) {
      next  # or return(), break, etc. depending on your context
    }
    ...
    for (j in idx_valid_intersection)
    {
      ...
      is_valid <- sapply(road_extended, function(line) {
        # Wrap in sfc and assign CRS before transforming
        line <- sf::st_sfc(line, crs = sf::st_crs(roads))  # or whatever original CRS is
        line <- sf::st_transform(line, sf::st_crs(bridge[["bridge"]]))

        inter <- sf::st_intersection(line, bridge[["bridge"]])
        !any(sf::st_geometry_type(inter) %in% c("LINESTRING", "MULTILINESTRING"))
      })
      if (is_valid==FALSE){
        next
      }
      road_extended <- road_extended[is_valid]
      ...
      if (length(idx_match_junc)==0) {next}
...
distance_line_intersection <- function(crossing_line, reference_line, reference_point)
{
  ...
  split_at_crossing <-
    lwgeom::st_split(reference_line, crossing_line) |>
      sf::st_collection_extract("LINESTRING")
  ...
  if (is.na(sf::st_crs(reference_line))) {
    sf::st_crs(reference_line) <- sf::st_crs(crossing_line)
  }

  if (is.na(sf::st_crs(reference_point))) {
    sf::st_crs(reference_point) <- sf::st_crs(reference_line)
  }
  ...
  return(dist_intersect_closest)
    ...
    warning("Skipping invalid line due to split error: ", conditionMessage(e))
    return(NULL)
  })
}
