...
find_road_edges = function(profiles, xc, accotement, thsd = 0.1, thz = 0.15)
{
  ...
  center_sd = median(gnd_profiles$ssdZ[abs(gnd_profiles$Xr - xc) <= 2.5], na.rm = TRUE)
  base_z <- 0.09
  ...
  if (length(right) > 0)
    ...
    pos1 = screen_profile(right, sdZ, avgZ - base_z, thsd, thz,
                   y1_center = center_sd, min_jump_factor = 1.8)
  else
    pos1 = nrow(gnd_profiles)

  if (length(left) > 0)
    # pos2 = screen_profile(left, sdZ, avgZ, thsd, thz)
    pos2 = screen_profile(left, sdZ, avgZ - base_z, thsd, thz,
                          y1_center = center_sd, min_jump_factor = 1.8)
  else
    pos2 = 1
...
#   return(pos)
# }
screen_profile = function(idx, y1, y2, thsd, thz, y1_center = NULL, min_jump_factor = 1.8)
{
  if (is.null(y2)) {
    y2 = rep(0, length(y1))
    thz = 0
  ...
  if (length(idx) <= 3) return(data.table::last(idx))

  pos = 0
  center_val = if (!is.null(y1_center)) y1_center else median(y1[idx[1:min(5,length(idx))]], na.rm=TRUE)

  # smoothed version helps avoid single noisy bins
  y1_smooth = zoo::rollapply(y1[idx], width=3, FUN=median, fill=NA, align="center")
  y1_smooth[is.na(y1_smooth)] = y1[idx][is.na(y1_smooth)]

  for (j in seq_along(idx)) {
    i = idx[j]

    if (anyNA(y1[i]) || anyNA(y2[i])) {
      if (pos == 0) pos = i
      next
    }

    # absolute rules (keep them)
    if (y1[i] > thsd || abs(y2[i]) > thz) {
      if (pos == 0) pos = i
      next   # don't break — allow later refinement
    }

    # new: relative worsening compared to center
    if (!is.na(center_val) &&
        y1[i] > center_val * min_jump_factor &&
        y1[i] > y1_smooth[max(1,j-1)] * 1.3) {
      if (pos == 0) pos = i
      break   # usually good place to stop
    }
  }

  if (pos == 0) pos = data.table::last(idx)
  return(pos)
}
