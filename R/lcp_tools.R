...
least_cost_path = function(las, centerline, dtm, conductivity, water, param,dl_model, ...)
{
  ...
  if (!is.null(dtm) && !is.null(conductivity) && conductivity != "v2" && conductivity != "v3" && conductivity != "v4") stop("'dtm' or 'conductivity' must be NULL", call. = FALSE)
  ...
  if (is.character(conductivity) && conductivity == "v3")
  {
    ...
    conductivity <- rasterize_conductivity3(las, centerline, dtm = dtm, param = param, return_all = FALSE, return_stack = FALSE)
  }
  if (is.character(conductivity) && conductivity == "v4")
  {
    verbose("Computing conductivity maps...\n")
    dots = list(...)
  ...
  # conductivity = enhance_curves_from_conductivity(conductivity, curve_threshold = 0.3, enhancement_factor = 1.5)
  # Handle bridge case:
  ...
  return(list(path, layers_lidar))
  # return(path)
}

enhance_curves_from_conductivity <- function(conduct_map, curve_threshold = 0.3, enhancement_factor = 1.5) {
  # Calculate gradients using Sobel operator
  display <- getOption("ALSroads.debug.finding")
  conduct_mask <- 1/conduct_map
  max_h = find_trough(conduct_mask)
  conduct_mask[conduct_mask < max_h] <- 1
  conduct_mask[conduct_mask >= max_h] <- 0



  gx <- terra::terrain(conduct_map, v="dx", unit="degrees")
  gy <- terra::terrain(conduct_map, v="dy", unit="degrees")

  # Calculate curvature using Laplacian
  laplacian <- terra::terrain(conduct_map, v="curvature")

  # Normalize curvature to 0-1 range
  curvature_norm <- abs(laplacian)
  min_val <- raster::minValue(curvature_norm)
  max_val <- raster::maxValue(curvature_norm)
  curvature_norm <- (curvature_norm - min_val) / (max_val - min_val)

  # Create curve mask where curvature is above threshold
  curve_mask <- curvature_norm
  curve_mask[curve_mask < curve_threshold] <- 0
  curve_mask[curve_mask >= curve_threshold] <- 1

  # Apply enhancement
  enhanced_map <- conduct_map
  enhanced_map[] <- conduct_map[] * (1 + (enhancement_factor - 1) * curve_mask[])* (1 + (enhancement_factor - 1) * conduct_mask[])
  enhanced_map[enhanced_map > 1] <- 1  # Ensure values stay between 0 and 1
  raster::plot(enhanced_map, col = viridis::viridis(25), main = "Conductivity enhanced")
...
find_path = function(trans, centerline, A, B, param)
{
  ...
  {
    ## ---- NEW PART ----
    vertices <- sf::st_cast(path, "POINT")

    cond_raster <- raster::raster(trans)

    radius <- param$segment_class %||% 3

    vbuf <- sf::st_buffer(vertices, radius)

    vertices$CONDUCTIVITY <- round(
      raster::extract(
        cond_raster,
        as(vbuf, "Spatial"),
        fun = mean,
